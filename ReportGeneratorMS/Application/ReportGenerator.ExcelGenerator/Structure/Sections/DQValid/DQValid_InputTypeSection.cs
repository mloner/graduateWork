...
{
    public class DQValid_InputTypeSection : ExcelReportSection
    {
        private new DQValid_InputTypeSectionModel SectionModel;

        public DQValid_InputTypeSection()
        {
            TemplateFileName = "DQValidReport/Template_DQValidReport_InputType.xlsx";
        }
        public override void InitSectionModel(string sectionModelJson)
        {
            base.InitSectionModel(sectionModelJson);
            SectionModel = JsonSerializer.Deserialize<DQValid_InputTypeSectionModel>(sectionModelJson);
        }

        public override void GenerateContent()
        {
            //find table
            var table = Worksheet.Table("Table_InputType");
            ...
            //insert data to table
             for (int i = 0; i < SectionModel.Data.Count; i++)
             {
                 ...
                 value = item["EcoscadaInputType"];
                 Worksheet.Cell(y + i, x++).Value = value;
                 
                 value = item["IsMeterBidirectional"];
                 Worksheet.Cell(y + i, x++).Value = value;
                 
                 value = item["IsMeterreading"];
                 Worksheet.Cell(y + i, x++).Value = value;
