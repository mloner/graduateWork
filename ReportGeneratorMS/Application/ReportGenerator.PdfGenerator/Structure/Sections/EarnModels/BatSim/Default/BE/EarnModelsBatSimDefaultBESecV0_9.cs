namespace ReportingFramework.Structure.Sections.EarnModels.BatSim.Default.BE
{
    public class EarnModelsBatSimDefaultBESecV0_9 : EarnModelsBatSimSec
    {
        ...
        public override void Generate()
        {
            ...
            //
            // //image
            // _name = $"{SubsectionTypeEnum.Image.Desc()}_EarnigModelsTariffs";
            // var image = GetReportObj(_sectionData, _name) as Image;
            // image.Init(_resParams, _template, _secParams.NumFmtr);
            // image.Path = $"{_resParams.Paths.SectionImages}/{image.Path}";
            // image.Title = ResHelper.GetResVal(_sectionResName, $"{_name}_Title", _ci);
            // AddImage(
